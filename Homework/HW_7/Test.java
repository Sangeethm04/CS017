...
public class Test {
    ...
    /**
        ...
        Time complexity: O(n x m) ~ O(n) m is constant, but n may grow in time
     */
    public static Country extremeEmission(ArrayList < Country > list, int year,
        boolean minMax, boolean type) {
        double extreme = 0.0;
        if (type)
            extreme = list.get(0).getEmission(year).next().getSecond();
        else
            extreme = list.get(0).getEmissionPerCapita(year).next().getSecond();
        Country extremeCountry = list.get(0);
        Iterator < Country > iter = list.listIterator();
        boolean changeExtreme = false;
        while (iter.hasNext()) { // O(m): m is the number of countries
            Country c = iter.next();
            //System.out.println("Country: " + c.getName());
            Iterator < Pair < Integer, Double >> iterEmission;
            if (type)
                iterEmission = c.getEmission(year); //O(n): n is the number of years
            else
                iterEmission = c.getEmissionPerCapita(year); // O(n): n is the number of years

            if (iterEmission != null) {
                Pair < Integer, Double > pair = iterEmission.next();
                if (minMax) {
                    changeExtreme = pair.getSecond() > extreme;
                } else {
                    changeExtreme = pair.getSecond() < extreme;
                }
                if (changeExtreme) {
                    extreme = pair.getSecond();
                    extremeCountry = c;
                }
            }
        }
    ...
    public static void printTenTopLowestEmissions(ArrayList < Country > list) {
        ...
        System.out.println("\nTen Top Countries with the Lowest Carbon Emission 2005-2019");
        System.out.printf("%-30s\t%s\n", "Country", "Carbon Emission (Tons)");
        for (int i = 0; i < 10; i++) {
            if (iterator.hasNext()) {
                Country c = iterator.next();
                System.out.printf("%-30s\t%-10.0f\n", c.getName(), c.getTotalEmissions());
            }
        }
    }
    /**
    ...
    public static void printTenTopHighestEmissions(ArrayList < Country > list) {
        ListIterator < Country > iterator = list.listIterator(list.size());
        System.out.println("\nTen Top Countries with the Highest Carbon Emission 2005-2019");
        System.out.printf("%-30s\t%s\n", "Country", "Carbon Emission (Tons)");
        for (int i = 0; i < 10; i++) {
            if (iterator.hasPrevious()) {
                Country c = iterator.previous();
                System.out.printf("%-30s\t%-10.0f\n", c.getName(), c.getTotalEmissions());
            }
        }
    }
